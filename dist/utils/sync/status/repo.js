...
import { INDEX_SYNC_MARKS_SQL, SYNC_STATUS_TABLES_SQL, } from "../../schema/createSyncStatus";
import { computeFilterKey } from "./filterKey";
const MILLISECOND = 1;
const OLD_DATE = "2000-01-01T00:00:00Z";
const addAMillisecond = (date, table) => {
...
const addAMillisecondToSyncStatus = (syncStatus) => {
    return {
        ...syncStatus,
        last_created_at: addAMillisecond(syncStatus.last_created_at, syncStatus.table_name),
        last_synced_at: addAMillisecond(syncStatus.last_synced_at, syncStatus.table_name),
        last_deleted_at: addAMillisecond(syncStatus.last_deleted_at, syncStatus.table_name),
    };
};
export async function ensureSyncMarksTable(db) {
...
export async function selectAndAddAMillisecond(db, table, tableFilters) {
...
export async function upsertMarks(db, row) {
    const { table_name, filter_key, filter_json = null, last_created_at = null, last_synced_at = null, last_deleted_at = null, } = row;
