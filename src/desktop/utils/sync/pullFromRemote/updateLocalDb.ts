...
*/
export async function upsertChunkData({
  tx,
  table,
  records,
}: {
  ...
  const localStamp = new Map<string, string | null>();
  ...
  for (let i = 0; i < toUpsert.length; i += rowsPerChunk) {
    ...
    try {
      await db.runAsync(
        `INSERT INTO ${table} (${colList}) VALUES ${rowPlaceholders} ON CONFLICT(id) DO UPDATE SET ${updateSet}`,
        values
      );
      for (const record of chunk) {
