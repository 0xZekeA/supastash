...
let dbInitPromise: Promise<SupastashSQLiteDatabase> | null = null;
...
export async function getSupastashDb(): Promise<SupastashSQLiteDatabase> {
  ...
  dbInitPromise = (async () => {
    ...
    const {
      sqliteClient: client,
      sqliteClientType: clientType,
      supastashMode,
    } = config;

    const resolveDbName =
      supastashMode === "ghost" ? `${config.dbName}_ghost` : config.dbName;
    ...
    switch (clientType) {
      case "expo": {
        ...
        db = await SQLiteAdapterExpo.openDatabaseAsync(
          resolveDbName,
          client as ExpoSQLiteClient
        );
        break;
      }
      case "rn-storage": {
        const { SQLiteAdapterStorage } = await import(
          "./adapters/rn_sqlite_storage"
        );
        db = await SQLiteAdapterStorage.openDatabaseAsync(
          resolveDbName,
          client as RNStorageSQLiteClient
        );
        break;
      }
      case "tauri": {
        const { SQLiteAdapterTauri } = await import("./adapters/tauri");
        db = await SQLiteAdapterTauri.openDatabaseAsync(
          resolveDbName,
          client as TauriSQLiteClient
        );
        break;
      }
      case "rn-nitro": {
        const { SQLiteAdapterNitro } = await import("./adapters/rn_nitro");
        db = await SQLiteAdapterNitro.openDatabaseAsync(
          resolveDbName,
          client as RNSqliteNitroClient
        );
        break;
      }
