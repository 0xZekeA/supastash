...
import { RpcFilterNode, RpcTableFilters } from "../../../types/rpcFilter.types";
 ...
 */
export async function updateRpcFilters(filters?: RpcTableFilters): Promise<void> {
  ...
  for (const [table, exists] of existence) {
    ...
    const nodes = (filters![table] ?? []) as RpcFilterNode[];
    if (!nodes.length) {
