...
export function classifyFailure(
  cfg: SupastashConfig<any>,
  code?: string | number
) {
  const p = cfg.syncPolicy ?? {};
  const n = Number(code);
  if (n === 409 || n === 412) return "HTTP";
...
async function singleUpsert(table: string, row: RowLike, supabase: any) {
...
async function fetchServerRowById(table: string, id: string, supabase: any) {
 ...
 * RPC Upsert
 * ────────────────────────────────────────────────────────────────────────── */
function setPending(table: string, rows: RowLike[]) {
  for (const row of rows) {
    setQueryStatus(row.id, table, "pending");
  }
}
async function rpcUpsert({
  table,
  rows,
  supabase,
}: {
  ...
  if (!cfg.pushRPCPath) {
    throw new Error(
      "pushRPCPath is not configured. Please configure it in the Supastash config. You can find more information in the Supastash docs: https://0xzekea.github.io/supastash/docs/sync-calls#%EF%B8%8F-pushrpcpath-custom-batch-sync-rpc"
    );
  }
  ...
  const mappedRows = new Map<string, RowLike>();
  for (const row of rows) {
    mappedRows.set(row.id, row);
  }

  const skipped: RowLike[] = [];
  const completed: RowLike[] = [];
...
async function rpcUpsertSingle({
  table,
  row,
  supabase,
  existsMap,
...
async function markSynced(table: string, ids: string[]) {
  await updateLocalSyncedAt(table, ids);
  for (const id of ids) setQueryStatus(id, table, "success");
  supastashEventBus.emit("updateSyncStatus");
}

async function acceptServerAndStopRetrying(table: string, id: string) {
...
function filterRowsByUpdatedAt(
  table: string,
  chunk: RowLike[],
  ...
  const cfg = getSupastashConfig();
  const defaultDate =
    cfg.fieldEnforcement?.autoFillDefaultISO ?? "1970-01-01T00:00:00Z";
...
async function handleRowFailure(
  ...
  if (klass === "NON_RETRYABLE") {
    ...
    if (action === "delete-local" || cfg.deleteConflictedRows) {
      ...
      logWarn(
        `Row ${row.id} on ${table} hit NON_RETRYABLE conflict → accepting server`,
        JSON.stringify(err)
      );
      await deleteLocalRow(table, row.id, supabase);
  ...
  if (klass === "FK_BLOCK") {
    ...
    log(
      `Row ${row.id} on ${table} blocked by missing parent (FK) → will retry`,
      JSON.stringify(err)
    );
  ...
  if (klass === "HTTP" || klass === "RETRYABLE" || klass === "UNKNOWN") {
    log(
      `Row ${row.id} on ${table} transient/HTTP error → scheduling retry`,
      JSON.stringify(err)
    );
    if (klass === "HTTP") {
      await deleteLocalRow(table, row.id, supabase);
...
async function deleteLocalRow(table: string, id: string, supabase: any) {
...
async function replaceLocalWithServer(table: string, serverRow: RowLike) {
...
export async function fetchRemoteHeadsChunked(
  table: string,
  ids: string[],
  supabase: any
) {
  ...
  for (let i = 0; i < ids.length; i += CHUNK) {
    ...
    for (const r of data ?? []) map.set(r.id, r.updated_at);
  }
  return map;
}
