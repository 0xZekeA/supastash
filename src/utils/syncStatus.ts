 ...
 */
export async function getLocalSyncLog(tableName: string): Promise<{
  table_name: string;
  lastSyncedAt: string;
  lastCreatedAt: string;
} | null> {
  const db = await getSupastashDb();
  const syncStatus = await db.getFirstAsync(
    `SELECT * FROM ${SYNC_STATUS_TABLE} WHERE table_name = ?`,
    [tableName]
  );
  const createdStatus = await db.getFirstAsync(
    `SELECT * FROM ${LAST_CREATED_TABLE} WHERE table_name = ?`,
    [tableName]
  );
  if (!syncStatus) {
    return null;
...
export async function setLocalSyncLog(
  ...
  if (lastSyncedAt) {
    const row = await db.getFirstAsync(
      `SELECT last_synced_at FROM ${SYNC_STATUS_TABLE} WHERE table_name=? LIMIT 1`,
      [tableName]
    );
    if (
      row?.last_synced_at &&
    ...
    await db.runAsync(
      `INSERT OR REPLACE INTO ${SYNC_STATUS_TABLE} (table_name, last_synced_at) VALUES (?, ?)`,
  ...
  if (lastCreatedAt) {
    const rowCreated = await db.getFirstAsync(
      `SELECT last_created_at FROM ${LAST_CREATED_TABLE} WHERE table_name=? LIMIT 1`,
      [tableName]
    );

    if (
      rowCreated?.last_created_at &&
      new Date(lastCreatedAt) > new Date(rowCreated.last_created_at)
    ) {
      lastCreatedAt = rowCreated.last_created_at;
    }
    await db.runAsync(
      `INSERT OR REPLACE INTO ${LAST_CREATED_TABLE} (table_name, last_created_at) VALUES (?, ?)`,
      [tableName, lastCreatedAt]
    );
  }
}
...
export async function clearLocalDeleteLog(tableName: string) {
  const db = await getSupastashDb();
  await db.runAsync(
    `DELETE FROM ${DELETED_STATUS_TABLE} WHERE table_name = ?`,
    [tableName]
  );
}
 ...
 */
export async function getLocalDeleteLog(tableName: string): Promise<{
  table_name: string;
  lastDeletedAt: string;
} | null> {
  const db = await getSupastashDb();
  const deleteStatus = await db.getFirstAsync(
    `SELECT * FROM ${DELETED_STATUS_TABLE} WHERE table_name = ?`,
    [tableName]
  );
  if (!deleteStatus) {
    return null;
...
export async function setLocalDeleteLog(
  ...
  const db = await getSupastashDb();
  await db.runAsync(
    `INSERT OR REPLACE INTO ${DELETED_STATUS_TABLE} (table_name, last_deleted_at) VALUES (?, ?)`,
    [tableName, lastDeletedAt]
  );
}
