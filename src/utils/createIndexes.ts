...
export async function createIdIndexes() {
  ...
  for (const table of tables) {
    ...
    try {
      ...
      const schemaRow = await db.getFirstAsync<{
        type: string | null;
        sql: string | null;
      }>(`SELECT type, sql FROM sqlite_schema WHERE name = ?1`, [table]);

      if (!schemaRow) continue; // not a real table in this db
      if (schemaRow.type !== "table") continue; // views, triggers, etc.
      ...
      if (!idCol) continue; // no "id" column → nothing to do
      ...
      try {
        ...
        await db.runAsync(
          `CREATE INDEX IF NOT EXISTS ${q(idxName)} ON ${q(table)}("id");`
        );
        await db.runAsync("COMMIT");
      } catch (e) {
        await db.runAsync("ROLLBACK");
        logError(`[index-skip] ${table}: ${(e as Error).message}`);
      }
    } catch (err) {
      logError(`[index-check-failed] ${table}: ${(err as Error).message}`);
    }
  }
}
