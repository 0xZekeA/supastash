...
export function useSupastashData<R = any>(
  ...
  const { dataMap, setDataMap, version, setVersion } = useDataState<R>(table);
  ...
  const { triggerRefresh, trigger, cancel, initialFetchAndSync, pushToUI } =
    fetchCalls(table, setDataMap, setVersion, options, initialized);

  useEffect(() => {
    if (!shouldFetch || (lazy && !initialized.current)) return;
  ...
  const data: R[] = useMemo(() => Array.from(dataMap.values()), [version]);
